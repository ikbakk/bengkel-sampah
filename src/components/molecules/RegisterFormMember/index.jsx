...
export default function SignUpMember({ bank }) {
  ...
  const registerUser = async (e) => {
    ...
    await axios
      .post(`/api/bank/${bankID}/members`, payload)
      .then(() => {
        ...
        >
          <div className="mb-1 flex flex-col gap-6">
            ...
            <Select
              ...
              value={data.bankID}
              label="Choose a bank"
              onChange={(value) => setData({ ...data, bankID: value })}
            >
              {bank &&
                bank.map((item) => (
                  <Option key={item.wasteBankID} value={item.wasteBankID}>
                    {item.name}
                  </Option>
                ))}
            </Select>
